...
__irq void CT16B0_IRQHandler(void)
{
  ...
  //User can add the directive pair of "#if 0" and "#endif" pair 
	...
	//MR9
	if (SN_CT16B0->MCTRL_b.MR9IE)				//Check if MR9 IE enables?
	{	
		if (iwRisStatus & mskCT16_MR9IF)	
		{
			iwCT16B0_IrqEvent |= mskCT16_MR9IF;		
			SN_CT16B0->IC = mskCT16_MR9IC;	//Clear MR9 match interrupt status
		}
	}	
	//CAP0
	...
	//BRK
	if (SN_CT16B0->BRKCTRL_b.BRKIE)		//Check if BRK IE enables?
	{
		if (iwRisStatus & mskCT16_BRKIF)	//BRK
		{
			iwCT16B0_IrqEvent |= mskCT16_BRKIF;
			SN_CT16B0->IC = mskCT16_BRKIC;	//Clear BRK interrupt status
		}	
	}
}
